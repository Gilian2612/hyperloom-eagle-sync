...
def is_in_sync_folder(item, folder_map):
    folder_ids = item.get("folders", [])
    folder_names = [folder_map.get(fid, "") for fid in folder_ids]
    return any(name == SYNC_FOLDER for name in folder_names)

def sync():
    print(f"Obteniendo items de Eagle (solo carpeta: '{SYNC_FOLDER}')...")
    items = get_eagle_items()
    folder_map = get_folder_map()
    last_sync = get_last_sync_time()

    filtered_items = [item for item in items if is_in_sync_folder(item, folder_map)]
