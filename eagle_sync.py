...
SYNC_FOLDER = "Shared to Supabase"
...
def get_sync_folder_ids(folders):
    """Recursively collect the ID of SYNC_FOLDER and all its descendant folder IDs."""
    sync_ids = set()

    def collect(folder_list, inside=False):
        for folder in folder_list:
            if folder["name"] == SYNC_FOLDER:
                inside = True
            if inside:
                sync_ids.add(folder["id"])
            if folder.get("children"):
                collect(folder["children"], inside)
...
def sync():
    print(f"Obteniendo items de Eagle (carpeta: '{SYNC_FOLDER}' y subcarpetas)...")
    items = get_eagle_items()

    folder_response = requests.get(f"{EAGLE_API}/api/folder/list")
    all_folders = folder_response.json().get("data", [])
    folder_map = {}
    def map_folders(folder_list):
        for folder in folder_list:
            folder_map[folder["id"]] = folder["name"]
            if folder.get("children"):
                map_folders(folder["children"])
    map_folders(all_folders)

    sync_ids = get_sync_folder_ids(all_folders)
    print(f"Carpetas incluidas en sync: {len(sync_ids)}")

    filtered_items = [
        item for item in items
        if any(fid in sync_ids for fid in item.get("folders", []))
    ]

    last_sync = get_last_sync_time()
    print(f"Encontrados {len(filtered_items)} items en '{SYNC_FOLDER}'")
